...
// My models
const User = require("./databases/mongodb/models/User.js");
const Course = require("./databases/mongodb/models/Course.js");
const Vertical = require("./databases/mongodb/models/Vertical.js");
const Unit = require("./databases/mongodb/models/Unit.js");
...
const fetchPerson = (req, res, next) => {
    ...
    if (!token) {
        return res.status(401).send({
            statusText: statusText.TOKEN_NOT_FOUND,
            isLoggedIn: false,
        });
    }
    ...
    try {
        ...
        res.status(401).send({
            statusText: statusText.INVALID_TOKEN,
            isLoggedIn: false,
        });
    }
};
...
const arePrereqSatisfied = async (req, res, next) => {
    ...
    if (!userDoc.isPassReset || !userDoc.isRegistered) {
        return res.status(403).json({
            statusText: statusText.PREREQ_NOT_SATISFIED,
            userDoc: userDoc,
        });
    }
